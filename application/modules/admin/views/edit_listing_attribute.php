...
<?php msg_alert(); ?> 
    ...
    <form method="post" action="<?php echo current_url()?>"  class="validation form-horizontal" />    	
        ...
        <div class="form-group"><label class="col-md-3 control-label">Product  MPN/ISBN</label>
            <div class="col-md-9">
                <input type="type" class="form-control" placeholder="MPN/ISBN Number" name="product_mpn_isbn" value="<?php if(!empty($listing_attributes->product_mpn_isbn)){ echo $listing_attributes->product_mpn_isbn; }else{ echo set_value('product_mpn_isbn'); } ?>" />
