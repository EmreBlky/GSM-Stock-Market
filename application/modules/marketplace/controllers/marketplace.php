...
class Marketplace extends MX_Controller {
    ...
    function listing() {   //$this->output->enable_profiler(TRUE);
        $member_id = $this->session->userdata('members_id');
    ...
    function get_listing_question($listing_id = 0) {
        ...
        if (!empty($question_asked)) {
            ...
            <table class="table table-striped table-bordered table-hover dataTables-example">
                <?php foreach ($question_asked as $value) { ?>
                    <tr>
                        <td>
                            <div class="row">
                                <div class="col-lg-6">
    ...
    public function shippings_to_couriers_data($ship_id = 0) {
        ...
        if ($shippings):
            ...
            if ($gstcd):
                foreach ($gstcd as $row):
                    ?>
                    <label class="checkbox-inline i-checks iCheck-helper"><input type="checkbox" value="<?php echo $row->courier_name; ?>" name="courier[]" <?php
                        if (!empty($_POST['courier']) && in_array($row->courier_name, $_POST['courier'])) {
                            echo'checked';
                        }
                        ?>/> <?php echo $row->courier_name; ?> </label>
                        <?php
                    endforeach;
                else:
        ...
        public function offer_status($id = '', $status = '0', $buyer_id) {
            $seller_id = $this->session->userdata('members_id');
            $offer_info = $this->marketplace_model->get_row('make_offer', array('id' => $id));
            if ($status == 1 && !empty($offer_info)) {
                $listing_id = $offer_info->listing_id;
                if ($listing_info = $this->marketplace_model->get_row('listing', array('id' => $listing_id))) {
                    $available_qty = $listing_info->qty_available;
                    $min_qty_order = $listing_info->min_qty_order;
                    if ($available_qty >= $min_qty_order) {
                        $current_total_qty = $available_qty - $offer_info->product_qty;
                        $this->marketplace_model->update('listing', array('qty_available' => $current_total_qty), array('id' => $listing_id));
                        if ($min_qty_order >= $current_total_qty) {
                            ...
                            $this->load->model('mailbox/mailbox_model', 'mailbox_model');
                            $this->mailbox_model->_insert($data1);
                        }
                    } else {
            ...
            } elseif ($status == 2 && !empty($offer_info)) {
        ...
        public function pay_asking_status($id = '', $negotiation_id = '', $status = '0', $buyer_id) {
            $seller_id = $this->session->userdata('members_id');
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <dl class="dl-horizontal">
                            ...
                            <dt>Make:</dt> <dd> <?php
                                if (!empty($deal_info->product_make)) {
                                    echo $deal_info->product_make;
                                }
                                ?></dd>
                            <dt>Model:</dt> <dd>  <?php
                                if (!empty($deal_info->product_model)) {
                                    echo $deal_info->product_model;
                                }
                                ?></dd>
                            <dt>Capacity:</dt> <dd>  <?php
                                if (!empty($deal_info->device_capacity)) {
                                    echo $deal_info->device_capacity;
                                }
                                ?></dd>
                            <dt>Colour:</dt> <dd> <?php
                                if (!empty($deal_info->product_color)) {
                                    echo $deal_info->product_color;
                                }
                                ?></dd>
                            <dt>Product Type:</dt> <dd> <?php
                                if (!empty($deal_info->product_type)) {
                                    echo $deal_info->product_type;
                                }
                                ?></dd>
                            <dt>Condition:</dt> <dd><?php
                                if (!empty($deal_info->condition)) {
                                    echo $deal_info->condition;
                                }
                                ?></dd>
                            <dt>Spec</dt> <dd>  <?php
                                if (!empty($deal_info->spec)) {
                                    echo $deal_info->spec;
                                }
                                ?></dd>
                        </dl>
                        <dl class="dl-horizontal">
                            ...
                            <dt>Buy Price:</dt> <dd>  <?php
                                if (!empty($deal_info->currency)) {
                                    echo currency_class($deal_info->currency);
                                }
                                if (!empty($deal_info->unit_price)) {
                                    echo $deal_info->unit_price;
                                }
                                ?></dd>
                            <dt>Product Type:</dt> <dd>  <?php
                                if (!empty($deal_info->product_type)) {
                    ...
                    <div class="col-lg-6">
                        <?php
                        if (!empty($deal_info->image1) || file_exists($deal_info->image1)) {
                        ...
                        <?php
                        if (!empty($deal_info->product_desc)) {
                            echo $deal_info->product_desc;
                        }
                        ?>
                    </div>
                    <?php
                    if ($get_order_info = $this->marketplace_model->get_row('make_offer', array('id' => $order_id))) {
    ...
    function counter_offer_negotiation() {
        if ($_POST) {
            ...
            if ($offerdetail = $this->marketplace_model->get_row('make_offer', array('id' => $offer_id))) {
                ...
                $sendermemid = $this->session->userdata('members_id');
                if ($sendermemid == $negotiation_detail->offer_given_by) {
                    $sendermemid = $negotiation_detail->offer_received_by;
